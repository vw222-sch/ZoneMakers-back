...
pub async fn post_admin_accept_zone_handler(
    ...
    let update_result = connection
        .execute("update zones set is_request = 0 where id = ?", (id,))
        .await;

    match update_result {
        Ok(_) => (StatusCode::OK, Json(json!({"message": "Zone request accepted"}))),
